...
namespace eCheque.MICO360.Core.Services
{
    public static class BackupService
    {
        ...
        {
            var dest = DatabaseService.GetSetting("BackupPath",
                Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "eCheque MICO360", "Backups"));
            Directory.CreateDirectory(dest);
            var src = DatabaseService.DbPath;
            if (string.IsNullOrEmpty(src) || !File.Exists(src)) throw new FileNotFoundException("No active database to back up.");
        ...
        {
            var path = DatabaseService.GetSetting("BackupPath",
                Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "eCheque MICO360", "Backups"));
