...
namespace eCheque.MICO360.Core.Services
{
    public static class CompanyService
    {
        ...
        public static void Initialize()
        {
            ...
            var masterDb = Path.Combine(AppPaths.DataFolder, "companies.db");
        ...
        private static void MigrateLegacyUsers()
        {
            try
            {
                ...
                if (!Directory.Exists(AppPaths.DataFolder)) return;
                foreach (var db in Directory.GetFiles(AppPaths.DataFolder, "company_*.db"))
                {
                    try
                    {
                        ...
                        if (imported > 0) { MasterAudit("SYSTEM", "Users Migrated", Path.GetFileName(db), $"Imported {imported} user(s)"); return; }
                    }
                    catch { }
                }
            }
        ...
        public static List<Company> GetAll()
        {
            ...
            while (r.Read())
                list.Add(new Company{Id=r.GetInt32(0),Name=r.GetString(1),TradeName=r.IsDBNull(2)?"":r.GetString(2),Address=r.IsDBNull(3)?"":r.GetString(3),Phone=r.IsDBNull(4)?"":r.GetString(4),Email=r.IsDBNull(5)?"":r.GetString(5),Currency=r.IsDBNull(6)?"OMR":r.GetString(6),CreatedDate=r.IsDBNull(7)?"":r.GetString(7),IsActive=r.GetInt32(8)==1});
            return list;
        }
        ...
        public static void Save(Company c)
        {
            ...
            if (c.Id == 0)
            {
                ...
                cmd.Parameters.AddWithValue("@n",c.Name);cmd.Parameters.AddWithValue("@t",c.TradeName);cmd.Parameters.AddWithValue("@a",c.Address);cmd.Parameters.AddWithValue("@p",c.Phone);cmd.Parameters.AddWithValue("@e",c.Email);cmd.Parameters.AddWithValue("@cu",c.Currency);cmd.Parameters.AddWithValue("@d",DateTime.Now.ToString("o"));cmd.ExecuteNonQuery();
            }
            else
            {
                ...
                cmd.Parameters.AddWithValue("@n",c.Name);cmd.Parameters.AddWithValue("@t",c.TradeName);cmd.Parameters.AddWithValue("@a",c.Address);cmd.Parameters.AddWithValue("@p",c.Phone);cmd.Parameters.AddWithValue("@e",c.Email);cmd.Parameters.AddWithValue("@cu",c.Currency);cmd.Parameters.AddWithValue("@id",c.Id);cmd.ExecuteNonQuery();
            }
        }
