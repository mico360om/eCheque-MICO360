...
namespace eCheque.MICO360.Core.Services
{
    public static class ChequeService
    {
        ...
        public static void CancelCheque(int id,string reason){using var conn=DatabaseService.GetConnection();using var cmd=new SqliteCommand("UPDATE ChequeRecords SET Status='Cancelled',CancellationReason=@r WHERE Id=@id",conn);cmd.Parameters.AddWithValue("@r",reason);cmd.Parameters.AddWithValue("@id",id);cmd.ExecuteNonQuery();DatabaseService.LogAudit(AuthService.CurrentUser?.Username??"","Cheque Cancelled",id.ToString(),reason);}
        ...
        static string DescribeChequeChanges(ChequeRecord? o,ChequeRecord n)
        {
            ...
            C("Cheque#",o.ChequeNumber,n.ChequeNumber);C("Date",o.ChequeDate.ToString("yyyy-MM-dd"),n.ChequeDate.ToString("yyyy-MM-dd"));
            C("Payee",o.PayeeName,n.PayeeName);C("Amount",o.Amount.ToString("N3"),n.Amount.ToString("N3"));
            C("Bank",o.BankName,n.BankName);C("Status",o.Status,n.Status);C("Memo",o.Remarks,n.Remarks);
        ...
        public static DashboardStats GetDashboardStats()
        {
            ...
            return new DashboardStats{
                Total=r.IsDBNull(0)?0:r.GetInt32(0),Printed=r.IsDBNull(1)?0:r.GetInt32(1),Draft=r.IsDBNull(2)?0:r.GetInt32(2),
                Cancelled=r.IsDBNull(3)?0:r.GetInt32(3),Voided=r.IsDBNull(4)?0:r.GetInt32(4),TodayPrinted=r.IsDBNull(5)?0:r.GetInt32(5),
                MonthPrinted=r.IsDBNull(6)?0:r.GetInt32(6),TotalAmount=r.IsDBNull(7)?0m:(decimal)r.GetDouble(7),
                MonthAmount=r.IsDBNull(8)?0m:(decimal)r.GetDouble(8),YearAmount=r.IsDBNull(9)?0m:(decimal)r.GetDouble(9)};
        }
        ...
        public static List<PrintHistory> GetPrintHistory(int? chequeId = null, int limit = 300)
        {
            ...
            using var r = cmd.ExecuteReader();
            while (r.Read())
                list.Add(new PrintHistory{
                    Id=r.GetInt32(0),ChequeId=r.IsDBNull(1)?0:r.GetInt32(1),ChequeNumber=r.IsDBNull(2)?"":r.GetString(2),
                    PrintedBy=r.IsDBNull(3)?"":r.GetString(3),PrintedDate=DateTime.TryParse(r.IsDBNull(4)?"":r.GetString(4),out var pd)?pd:DateTime.Now,
                    Reason=r.IsDBNull(5)?"":r.GetString(5),IsReprint=!r.IsDBNull(6)&&r.GetInt32(6)==1,
                    PayeeName=r.IsDBNull(7)?"":r.GetString(7),BankName=r.IsDBNull(8)?"":r.GetString(8),Amount=r.IsDBNull(9)?0:(decimal)r.GetDouble(9)});
            return list;
        }

        public static string GetNextChequeNumber(int profileId){using var conn=DatabaseService.GetConnection();using var cmd=new SqliteCommand("SELECT LastChequeNumber FROM ChequeProfiles WHERE Id=@id",conn);cmd.Parameters.AddWithValue("@id",profileId);var current=Convert.ToInt32(cmd.ExecuteScalar());return (current+1).ToString("D6");}
        public static void IncrementChequeNumber(int profileId){using var conn=DatabaseService.GetConnection();using var cmd=new SqliteCommand("UPDATE ChequeProfiles SET LastChequeNumber=LastChequeNumber+1 WHERE Id=@id",conn);cmd.Parameters.AddWithValue("@id",profileId);cmd.ExecuteNonQuery();}

        public static void SaveProfilePositions(ChequeProfile p)
        {
            using var conn=DatabaseService.GetConnection();
            using var cmd=new SqliteCommand("UPDATE ChequeProfiles SET DateX=@dx,DateY=@dy,PayeeX=@px,PayeeY=@py,AmountNumX=@ax,AmountNumY=@ay,AmountWordsX=@wx,AmountWordsY=@wy,ChequeNumX=@nx,ChequeNumY=@ny WHERE Id=@id",conn);
            cmd.Parameters.AddWithValue("@dx",p.DateX);cmd.Parameters.AddWithValue("@dy",p.DateY);cmd.Parameters.AddWithValue("@px",p.PayeeX);cmd.Parameters.AddWithValue("@py",p.PayeeY);cmd.Parameters.AddWithValue("@ax",p.AmountNumX);cmd.Parameters.AddWithValue("@ay",p.AmountNumY);cmd.Parameters.AddWithValue("@wx",p.AmountWordsX);cmd.Parameters.AddWithValue("@wy",p.AmountWordsY);cmd.Parameters.AddWithValue("@nx",p.ChequeNumX);cmd.Parameters.AddWithValue("@ny",p.ChequeNumY);cmd.Parameters.AddWithValue("@id",p.Id);
            cmd.ExecuteNonQuery();
        ...
        /// <summary>Persists the visual designer output: scanned template + field layout (JSON) + the legacy X/Y for the print fallback.</summary>
        public static void SaveLayout(ChequeProfile p)
        {
            using var conn=DatabaseService.GetConnection();
            using var cmd=new SqliteCommand("UPDATE ChequeProfiles SET BackgroundImage=@bg,FieldsJson=@fj,DateX=@dx,DateY=@dy,PayeeX=@px,PayeeY=@py,AmountNumX=@ax,AmountNumY=@ay,AmountWordsX=@wx,AmountWordsY=@wy,ChequeNumX=@nx,ChequeNumY=@ny WHERE Id=@id",conn);
